...
#if (CUSTOM_MEMORY_MANAGEMENT_FILE_HANDLING > 0)
#define FILE FIL
#define fopen xml_fopen
#define fclose xml_fclose
#define fread xml_fread
#endif  //(CUSTOM_MEMORY_MANAGEMENT_FILE_HANDLING > 0)
...
//    delete [] _charBuffer;  //- no more copying to _charBuffer
    _charBuffer = 0;
...
#if (CUSTOM_MEMORY_MANAGEMENT_FILE_HANDLING > 0)
    const long filelength = f_size(fp);
#else
    fseek( fp, 0, SEEK_SET );
...
//    _charBuffer[len] = 0;
    ...
    //TODO: Review - Extra copying has been removed here!
    _charBuffer = (char*) p;
...
#else
                            f_printf( _fp, "&%s;", entities[i].pattern );
#endif  //(CUSTOM_MEMORY_MANAGEMENT_FILE_HANDLING == 0)
