...
#include "opcua_call_response_t.h"
...
Status_t CallResponse_t::EncodeBody(ICodec_t& encoder, IBuffer_t& buffer) const
{
...
Status_t CallResponse_t::Decode(const IBuffer_t& buffer, ICodec_t& decoder, CallResponse_t& result)
{
  ...
  status = ArrayUA_t<CallMethodResult_t>::Decode(buffer, decoder, result.Results());
  if (status.IsBad())
  {
    return status;
  }
