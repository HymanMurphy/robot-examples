...
Status_t ModifySubscriptionRequest_t::EncodeBody(ICodec_t& encoder, IBuffer_t& buffer) const
{
  ...
  status = requestedPublishingInterval.Encode(encoder, buffer);
  if (status.IsBad())
  {
    return status;
  }
...
Status_t ModifySubscriptionRequest_t::Decode(const IBuffer_t& buffer, ICodec_t& decoder, ModifySubscriptionRequest_t& result)
{
