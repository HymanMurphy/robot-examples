...
Status_t TransferSubscriptionsRequest_t::EncodeBody(ICodec_t& encoder, IBuffer_t& buffer) const
{
  ...
  status = sendInitialValues.Encode(encoder, buffer);
  if (status.IsBad())
  {
    return status;
  }
...
Status_t TransferSubscriptionsRequest_t::Decode(const IBuffer_t& buffer, ICodec_t& decoder, TransferSubscriptionsRequest_t& result)
{
  ...
  status = Boolean_t::Decode(buffer, decoder, result.SendInitialValues());
  if (status.IsBad())
  {
    return status;
  }
