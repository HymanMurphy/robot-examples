...
#include "opcua_modify_subscription_response_t.h"
...
Status_t ModifySubscriptionResponse_t::EncodeBody(ICodec_t& encoder, IBuffer_t& buffer) const
{
#if (UASDK_INCLUDE_SERVER > 0)
  Status_t status = revisedPublishingInterval.Encode(encoder, buffer);
  if (status.IsBad())
  {
    return status;
  }
...
Status_t ModifySubscriptionResponse_t::Decode(const IBuffer_t& buffer, ICodec_t& decoder, ModifySubscriptionResponse_t& result)
{
