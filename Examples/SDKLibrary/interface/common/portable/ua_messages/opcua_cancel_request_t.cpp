...
Status_t CancelRequest_t::EncodeBody(ICodec_t& encoder, IBuffer_t& buffer) const
{
#if (UASDK_INCLUDE_CLIENT > 0)
  Status_t status = requestHandle.Encode(encoder, buffer);
  if (status.IsBad())
  {
    return status;
  }
...
Status_t CancelRequest_t::Decode(const IBuffer_t& buffer, ICodec_t& decoder, CancelRequest_t& result)
{
