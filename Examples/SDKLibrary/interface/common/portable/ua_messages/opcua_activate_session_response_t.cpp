...
#include "opcua_activate_session_response_t.h"

namespace uasdk
{

Status_t ActivateSessionResponse_t::EncodeBody(ICodec_t& encoder, IBuffer_t& buffer) const
{
#if (UASDK_INCLUDE_SERVER > 0)
  Status_t status = serverNonce.Encode(encoder, buffer);
  if (status.IsBad())
  {
    return status;
  }
...
Status_t ActivateSessionResponse_t::Decode(const IBuffer_t& buffer, ICodec_t& decoder, ActivateSessionResponse_t& result)
{
