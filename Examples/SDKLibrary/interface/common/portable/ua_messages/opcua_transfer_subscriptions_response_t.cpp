...
#include "opcua_transfer_subscriptions_response_t.h"
...
Status_t TransferSubscriptionsResponse_t::EncodeBody(ICodec_t& encoder, IBuffer_t& buffer) const
{
...
Status_t TransferSubscriptionsResponse_t::Decode(const IBuffer_t& buffer, ICodec_t& decoder, TransferSubscriptionsResponse_t& result)
{
  ...
  status = ArrayUA_t<TransferResult_t>::Decode(buffer, decoder, result.Results());
  if (status.IsBad())
  {
    return status;
  }
