...
#include "opcua_modify_monitored_items_response_t.h"
...
Status_t ModifyMonitoredItemsResponse_t::EncodeBody(ICodec_t& encoder, IBuffer_t& buffer) const
{
...
Status_t ModifyMonitoredItemsResponse_t::Decode(const IBuffer_t& buffer, ICodec_t& decoder, ModifyMonitoredItemsResponse_t& result)
{
  ...
  status = ArrayUA_t<MonItemModifyResult_t>::Decode(buffer, decoder, result.Results());
  if (status.IsBad())
  {
    return status;
  }
