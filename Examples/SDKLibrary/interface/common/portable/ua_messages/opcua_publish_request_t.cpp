...
Status_t PublishRequest_t::EncodeBody(ICodec_t& encoder, IBuffer_t& buffer) const
{
#if (UASDK_INCLUDE_CLIENT > 0)
  Status_t status = subscriptionAcks.Encode(encoder, buffer);
  if (status.IsBad())
  {
    return status;
  }
...
Status_t PublishRequest_t::Decode(const IBuffer_t& buffer, ICodec_t& decoder, PublishRequest_t& result)
{
  ...
  status = ArrayUA_t<SubscriptionAcknowledgment_t>::Decode(buffer, decoder, result.SubscriptionAcks());
  if (status.IsBad())
  {
    return status;
  }
