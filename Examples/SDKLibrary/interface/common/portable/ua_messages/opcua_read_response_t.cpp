...
#include "opcua_read_response_t.h"

namespace uasdk
{

Status_t ReadResponse_t::EncodeBody(ICodec_t& encoder, IBuffer_t& buffer) const
{
...
Status_t ReadResponse_t::Decode(const IBuffer_t& buffer, ICodec_t& decoder, ReadResponse_t& result)
{
  ...
  status = ArrayUA_t<DataValue_t>::Decode(buffer, decoder, result.Results());
  if (status.IsBad())
  {
    return status;
  }
