...
#include "opcua_translate_browse_paths_response_t.h"

namespace uasdk
{

Status_t TranslateBrowsePathsResponse_t::EncodeBody(ICodec_t& encoder, IBuffer_t& buffer) const
{
...
Status_t TranslateBrowsePathsResponse_t::Decode(const IBuffer_t& buffer, ICodec_t& decoder, TranslateBrowsePathsResponse_t& result)
{
  ...
  status = ArrayUA_t<BrowsePathResult_t>::Decode(buffer, decoder, result.Results());
  if (status.IsBad())
  {
    return status;
  }
