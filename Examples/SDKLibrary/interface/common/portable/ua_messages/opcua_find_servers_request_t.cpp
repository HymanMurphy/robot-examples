...
Status_t FindServersRequest_t::EncodeBody(ICodec_t& encoder, IBuffer_t& buffer) const
{
  ...
  status = serverUris.Encode(encoder, buffer);
  if (status.IsBad())
  {
    return status;
  }
...
Status_t FindServersRequest_t::Decode(const IBuffer_t& buffer, ICodec_t& decoder, FindServersRequest_t& result)
{
  ...
  status = ArrayUA_t<String_t>::Decode(buffer, decoder, result.ServerUris());
  if (status.IsBad())
  {
    return status;
  }
