...
Status_t GetEndpointsRequest_t::EncodeBody(ICodec_t& encoder, IBuffer_t& buffer) const
{
  ...
  status = profileUris.Encode(encoder, buffer);
  if (status.IsBad())
  {
    return status;
  }
...
Status_t GetEndpointsRequest_t::Decode(const IBuffer_t& buffer, ICodec_t& decoder, GetEndpointsRequest_t& result)
{
  ...
  status = ArrayUA_t<String_t>::Decode(buffer, decoder, result.ProfileUris());
  if (status.IsBad())
  {
    return status;
  }
