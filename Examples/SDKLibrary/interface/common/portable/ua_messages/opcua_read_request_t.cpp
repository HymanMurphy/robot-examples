...
Status_t ReadRequest_t::EncodeBody(ICodec_t& encoder, IBuffer_t& buffer) const
{
#if (UASDK_INCLUDE_CLIENT > 0)
  Status_t status = maxAge.Encode(encoder, buffer);
  if (status.IsBad())
  {
    return status;
  }
  ...
  status = nodesToRead.Encode(encoder, buffer);
  if (status.IsBad())
  {
    return status;
  }
...
Status_t ReadRequest_t::Decode(const IBuffer_t& buffer, ICodec_t& decoder, ReadRequest_t& result)
{
  ...
  status = Duration_t::Decode(buffer, decoder, result.MaxAge());
  if (status.IsBad())
  {
    return status;
  }
  ...
  status = ArrayUA_t<ReadValueId_t>::Decode(buffer, decoder, result.NodesToRead());
  if (status.IsBad())
  {
    return status;
  }
