...
UA_DEFINE_RUNTIME_TYPE(ContentFilterElement_LessThanOrEqual_t, ContentFilterElement_t);

ContentFilterElement_LessThanOrEqual_t::ContentFilterElement_LessThanOrEqual_t() : ContentFilterElement_t(FilterOperator_t::FILTER_OP_LESSTHANOREQUAL)
{
}

ContentFilterElement_LessThanOrEqual_t::~ContentFilterElement_LessThanOrEqual_t()
{
}

uint32_t  ContentFilterElement_LessThanOrEqual_t::MinimumOperandCount(void) const
{
	return 2;
}

uint32_t  ContentFilterElement_LessThanOrEqual_t::MaximumOperandCount(void) const
{
	return 2;
}

Status_t ContentFilterElement_LessThanOrEqual_t::ApplyFilterElement(const INode_t& node, IContentFilter_t* filter, IntrusivePtr_t<IAttributeAccess_t>& eventAttributes, IntrusivePtr_t<IServerConfiguration_t> configuration, IntrusivePtr_t<BaseDataType_t>& evaluationResult, IntrusivePtr_t<ContentFilterElementResult_t>& elementResult, bool diagnosticsRequested)
{
  ...
  if(value1->TypeId(namespaceIndex1) != value2->TypeId(namespaceIndex2))
  {
    ...
    if(status.IsGood())
    {
      ...
      {
        if(*value1 > *dest)//<= operator is not available for all types. Hence inverse of (>) is similar to <=
        {
					returnResult = false;
        }
      }
      ...
      {
        if(*dest > *value2)//<= operator is not available for all types. Hence inverse of (>) is similar to <=
        {
					returnResult = false;
        }
      }
  ...
  else
  {
    ...
    if(*value1 > *value2)//<= operator is not available for all types. Hence inverse of (>) is similar to <=
    {
			returnResult = false;
    }
  }
...
Status_t ContentFilterElement_LessThanOrEqual_t::CopyTo(IntrusivePtr_t<BaseDataType_t>& destination) const
{
	return CopyToDestination(*this, destination);
}

Status_t ContentFilterElement_LessThanOrEqual_t::CopyFrom(const BaseDataType_t& source)
{
	IntrusivePtr_t<const ContentFilterElement_LessThanOrEqual_t> ptr = RuntimeCast<const ContentFilterElement_LessThanOrEqual_t*>(source);
	if (!ptr.is_set())
	{
...
Status_t ContentFilterElement_LessThanOrEqual_t::CopyFrom(const ContentFilterElement_LessThanOrEqual_t& source)
{
