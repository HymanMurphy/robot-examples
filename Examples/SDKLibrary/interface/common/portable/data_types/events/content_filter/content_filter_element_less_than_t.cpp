...
UA_DEFINE_RUNTIME_TYPE(ContentFilterElement_LessThan_t, ContentFilterElement_t);

ContentFilterElement_LessThan_t::ContentFilterElement_LessThan_t() : ContentFilterElement_t(FilterOperator_t::FILTER_OP_LESSTHAN)
{
}

ContentFilterElement_LessThan_t::~ContentFilterElement_LessThan_t()
{
}

uint32_t  ContentFilterElement_LessThan_t::MinimumOperandCount(void) const
{
	return 2;
}

uint32_t  ContentFilterElement_LessThan_t::MaximumOperandCount(void) const
{
	return 2;
}

Status_t ContentFilterElement_LessThan_t::ApplyFilterElement(const INode_t& node, IContentFilter_t* filter, IntrusivePtr_t<IAttributeAccess_t>& eventAttributes, IntrusivePtr_t<IServerConfiguration_t> configuration, IntrusivePtr_t<BaseDataType_t>& evaluationResult, IntrusivePtr_t<ContentFilterElementResult_t>& elementResult, bool diagnosticsRequested)
{
  ...
  if(value1->TypeId(namespaceIndex1) != value2->TypeId(namespaceIndex2))
  {
    ...
    if(status.IsGood())
    {
      ...
      {
        if((*value1 > *dest || *value1 == *dest))//< operator is not available for all types. Hence inverse of (> || ==) is similar to <
        {
					returnResult = false;
        }
      }
      ...
      {
        if((*dest > *value2 || *dest == *value2))//< operator is not available for all types. Hence inverse of (> || ==) is similar to <
        {
					returnResult = false;
        }
      }
  ...
  {
    //same type case, directly compare
    if((*value1 > *value2 || *value1 == *value2))//< operator is not available for all types. Hence inverse of (> || ==) is similar to <
    {
			returnResult = false;
    }
  }
...
Status_t  ContentFilterElement_LessThan_t::CopyTo(IntrusivePtr_t<BaseDataType_t>& destination) const
{
	return CopyToDestination(*this, destination);
}

Status_t  ContentFilterElement_LessThan_t::CopyFrom(const BaseDataType_t& source)
{
	IntrusivePtr_t<const  ContentFilterElement_LessThan_t> ptr = RuntimeCast<const ContentFilterElement_LessThan_t*>(source);
	if (!ptr.is_set())
	{
...
Status_t  ContentFilterElement_LessThan_t::CopyFrom(const  ContentFilterElement_LessThan_t& source)
{
