...
UA_DEFINE_RUNTIME_TYPE(ContentFilterElement_BitwiseOr_t, ContentFilterElement_t);

ContentFilterElement_BitwiseOr_t::ContentFilterElement_BitwiseOr_t() : ContentFilterElement_t(FilterOperator_t::FILTER_OP_BITWISEOR)
{
}

ContentFilterElement_BitwiseOr_t::~ContentFilterElement_BitwiseOr_t()
{
}

uint32_t  ContentFilterElement_BitwiseOr_t::MinimumOperandCount(void) const
{
	return 2;
}

uint32_t  ContentFilterElement_BitwiseOr_t::MaximumOperandCount(void) const
{
	return 2;
}

Status_t ContentFilterElement_BitwiseOr_t::ApplyFilterElement(const INode_t& node, IContentFilter_t* filter, IntrusivePtr_t<IAttributeAccess_t>& eventAttributes, IntrusivePtr_t<IServerConfiguration_t> configuration, IntrusivePtr_t<BaseDataType_t>& evaluationResult, IntrusivePtr_t<ContentFilterElementResult_t>& elementResult, bool diagnosticsRequested)
{
	bool isNULL = false;
	...
	if (status.IsGood())
	{
		...
		{
			//Perform Bitwise OR operation and set the output result
			status = PerformBitwiseOrOperation(value1, value2, evaluationResult);
		}
	}
...
Status_t ContentFilterElement_BitwiseOr_t::CopyTo(IntrusivePtr_t<BaseDataType_t>& destination) const
{
	return CopyToDestination(*this, destination);
}

Status_t ContentFilterElement_BitwiseOr_t::CopyFrom(const BaseDataType_t& source)
{
	IntrusivePtr_t<const ContentFilterElement_BitwiseOr_t> ptr = RuntimeCast<const ContentFilterElement_BitwiseOr_t*>(source);
	if (!ptr.is_set())
	{
...
Status_t ContentFilterElement_BitwiseOr_t::CopyFrom(const ContentFilterElement_BitwiseOr_t& source)
{
...
Status_t ContentFilterElement_BitwiseOr_t::PerformBitwiseOrOperation(IntrusivePtr_t<BaseDataType_t> value1, IntrusivePtr_t<BaseDataType_t> value2, IntrusivePtr_t<BaseDataType_t>& evaluationResult)
{
