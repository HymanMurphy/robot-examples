...
#include "trip_alarm_t.h"

#if ((UASDK_INCLUDE_EVENTS > 0) && (UASDK_INCLUDE_ALARMS_AND_CONDITIONS > 0))

namespace uasdk
{
UA_DEFINE_RUNTIME_TYPE(TripAlarm_t, OffNormalAlarm_t);

/*
 * Private Function
 */
Status_t TripAlarm_t::InitialiseConditionDataAttributes(void)
{
  return OpcUa_Good;
}
...
Status_t TripAlarm_t::EvaluteState(uint32_t methodId)
{
...
Status_t TripAlarm_t::InitialiseConditionData(IntrusivePtr_t<ConditionTypeData_t> conditionData_, uint32_t conditionClassId, String_t& conditionName, bool isAllocated)
{
  if (conditionData_.is_set())
  {
    conditionData = RuntimeCast<TripAlarmTypeData_t*>(*conditionData_);
  ...
  {
    conditionData = new SafeRefCount_t<TripAlarmTypeData_t>();
  ...
  return TripAlarm_t::InitialiseConditionDataAttributes();
}

Status_t TripAlarm_t::GetEventData(IntrusivePtr_t<IAttributeAccess_t>& data, IntrusivePtr_t<NodeId_t>& sourceNodeId)
{
  Status_t status;
...
* Public Functions
*/
TripAlarm_t::TripAlarm_t()
{}

TripAlarm_t::~TripAlarm_t()
{}

Status_t TripAlarm_t::Initialise(IntrusivePtr_t<IServerConfiguration_t> configuration_, NodeId_t& conditionId_, uint32_t conditionClassId, String_t& conditionName)
{
  return OffNormalAlarm_t::Initialise(configuration_, conditionId_, conditionClassId, conditionName);
}

Status_t TripAlarm_t::SetMethodListener(IntrusivePtr_t<ITripAlarmMethodListener_t> value)
{
  conditionMethodListener = value;
  return OffNormalAlarm_t::SetMethodListener(value);
}

Status_t TripAlarm_t::CallMethodBegin(IntrusivePtr_t<const NodeId_t> sessionId,
...
Status_t TripAlarm_t::ReadValueAttribute(
  ...
  IntrusivePtr_t<DiagnosticInfo_t>& diagnosticInfo)
{
  ...
  //No variable is available for TripAlarm, then call the parent class ReadValueAttribute
