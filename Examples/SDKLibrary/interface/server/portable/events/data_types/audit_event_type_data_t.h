...
#ifndef _AUDITEVENTTYPEDATA_T_H_
#define _AUDITEVENTTYPEDATA_T_H_
...
class AuditEventTypeData_t : public BaseEventTypeData_t
{
public:
	UA_DECLARE_RUNTIME_TYPE(AuditEventTypeData_t);
	
	static const uint32_t TYPE_ID = OpcUaId_AuditEventType;
	
	AuditEventTypeData_t();
	virtual ~AuditEventTypeData_t();
	
	virtual Status_t CopyTo(IntrusivePtr_t<BaseDataType_t>& destination) const;
	virtual Status_t CopyFrom(const BaseDataType_t& source);
	Status_t CopyFrom(const AuditEventTypeData_t& source);
...
public:
	//Accessors
	IntrusivePtr_t<const UtcTime_t> ActionTimeStamp(void) const {return actionTimeStamp;}
	
	IntrusivePtr_t<UtcTime_t> &ActionTimeStamp(void) {return actionTimeStamp;}
	
	
	
	IntrusivePtr_t<const Boolean_t> Status(void) const {return status;}
	
	IntrusivePtr_t<Boolean_t> &Status(void) {return status;}
	
	
	
	IntrusivePtr_t<const String_t> ServerId(void) const {return serverId;}
	
	IntrusivePtr_t<String_t> &ServerId(void) {return serverId;}
	
	
	
	IntrusivePtr_t<const String_t> ClientAuditEntryId(void) const {return clientAuditEntryId;}
	
	IntrusivePtr_t<String_t> &ClientAuditEntryId(void) {return clientAuditEntryId;}
...
#endif /* _AUDITEVENTTYPEDATA_T_H_ */
