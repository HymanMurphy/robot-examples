...
#ifndef _ACKNOWLEDGEABLECONDITIONTYPEDATA_T_H_
#define _ACKNOWLEDGEABLECONDITIONTYPEDATA_T_H_
...
class AcknowledgeableConditionTypeData_t : public ConditionTypeData_t
{
public:
	UA_DECLARE_RUNTIME_TYPE(AcknowledgeableConditionTypeData_t);
	
	static const uint32_t TYPE_ID = OpcUaId_AcknowledgeableConditionType;
	
	AcknowledgeableConditionTypeData_t();
	virtual ~AcknowledgeableConditionTypeData_t();
	
	virtual Status_t CopyTo(IntrusivePtr_t<BaseDataType_t>& destination) const;
	virtual Status_t CopyFrom(const BaseDataType_t& source);
	Status_t CopyFrom(const AcknowledgeableConditionTypeData_t& source);
...
private:
	...
	IntrusivePtr_t<TwoStateVariableType_t> ackedState;
	
	IntrusivePtr_t<TwoStateVariableType_t> confirmedState;
	
	

public:
	//Accessors
	IntrusivePtr_t<const TwoStateVariableType_t> AckedState(void) const { return ackedState;}
	
	IntrusivePtr_t<TwoStateVariableType_t> &AckedState(void) { return ackedState;}
	
	IntrusivePtr_t<const TwoStateVariableType_t> ConfirmedState(void) const { return confirmedState;}
	
	IntrusivePtr_t<TwoStateVariableType_t> &ConfirmedState(void) { return confirmedState;}
...
#endif /* _ACKNOWLEDGEABLECONDITIONTYPEDATA_T_H_ */
