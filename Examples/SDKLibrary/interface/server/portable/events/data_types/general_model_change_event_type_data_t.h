...
#ifndef _GENERALMODELCHANGEEVENTTYPEDATA_T_H_
#define _GENERALMODELCHANGEEVENTTYPEDATA_T_H_
...
class GeneralModelChangeEventTypeData_t : public BaseModelChangeEventTypeData_t
{
public:
	UA_DECLARE_RUNTIME_TYPE(GeneralModelChangeEventTypeData_t);
	
	static const uint32_t TYPE_ID = OpcUaId_GeneralModelChangeEventType;
	
	GeneralModelChangeEventTypeData_t();
	virtual ~GeneralModelChangeEventTypeData_t();
	
	virtual Status_t CopyTo(IntrusivePtr_t<BaseDataType_t>& destination) const;
	virtual Status_t CopyFrom(const BaseDataType_t& source);
	Status_t CopyFrom(const GeneralModelChangeEventTypeData_t& source);
...
private:
	...
	IntrusivePtr_t<NodeId_t> changes_Affected;
	
	IntrusivePtr_t<NodeId_t> changes_AffectedType;
	
	IntrusivePtr_t<Byte_t> changes_Verb;
	
	ArrayUA_t<ModelChangeStructureDataType_t> changes;
	
	

public:
	//Accessors
	IntrusivePtr_t<const NodeId_t> Changes_Affected(void) const {return changes_Affected;}
	
	IntrusivePtr_t<NodeId_t> &Changes_Affected(void) {return changes_Affected;}
	
	
	
	IntrusivePtr_t<const NodeId_t> Changes_AffectedType(void) const {return changes_AffectedType;}
	
	IntrusivePtr_t<NodeId_t> &Changes_AffectedType(void) {return changes_AffectedType;}
	
	
	
	IntrusivePtr_t<const Byte_t> Changes_Verb(void) const {return changes_Verb;}
	
	IntrusivePtr_t<Byte_t> &Changes_Verb(void) {return changes_Verb;}
	
	
	
	ArrayUA_t<ModelChangeStructureDataType_t> &Changes(void) {return changes;}
...
#endif /* _GENERALMODELCHANGEEVENTTYPEDATA_T_H_ */
