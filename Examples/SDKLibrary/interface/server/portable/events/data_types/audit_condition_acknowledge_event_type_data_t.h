...
#ifndef _AUDITCONDITIONACKNOWLEDGEEVENTTYPEDATA_T_H_
#define _AUDITCONDITIONACKNOWLEDGEEVENTTYPEDATA_T_H_
...
class AuditConditionAcknowledgeEventTypeData_t : public AuditConditionEventTypeData_t
{
public:
	UA_DECLARE_RUNTIME_TYPE(AuditConditionAcknowledgeEventTypeData_t);
	
	static const uint32_t TYPE_ID = OpcUaId_AuditConditionAcknowledgeEventType;
	
	AuditConditionAcknowledgeEventTypeData_t();
	virtual ~AuditConditionAcknowledgeEventTypeData_t();
	
	virtual Status_t CopyTo(IntrusivePtr_t<BaseDataType_t>& destination) const;
	virtual Status_t CopyFrom(const BaseDataType_t& source);
	Status_t CopyFrom(const AuditConditionAcknowledgeEventTypeData_t& source);
...
private:
	...
	IntrusivePtr_t<ByteString_t> affectedEventId;
...
public:
	//Accessors
	IntrusivePtr_t<const ByteString_t> AffectedEventId(void) const {return affectedEventId;}
	
	IntrusivePtr_t<ByteString_t> &AffectedEventId(void) {return affectedEventId;}
	
	
	
	IntrusivePtr_t<const LocalizedText_t> Comment(void) const {return comment;}
	
	IntrusivePtr_t<LocalizedText_t> &Comment(void) {return comment;}
...
#endif /* _AUDITCONDITIONACKNOWLEDGEEVENTTYPEDATA_T_H_ */
