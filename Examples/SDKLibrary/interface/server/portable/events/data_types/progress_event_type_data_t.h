...
#ifndef _PROGRESSEVENTTYPEDATA_T_H_
#define _PROGRESSEVENTTYPEDATA_T_H_
...
class ProgressEventTypeData_t : public BaseEventTypeData_t
{
public:
	UA_DECLARE_RUNTIME_TYPE(ProgressEventTypeData_t);
	
	static const uint32_t TYPE_ID = OpcUaId_ProgressEventType;
	
	ProgressEventTypeData_t();
	virtual ~ProgressEventTypeData_t();
	
	virtual Status_t CopyTo(IntrusivePtr_t<BaseDataType_t>& destination) const;
	virtual Status_t CopyFrom(const BaseDataType_t& source);
	Status_t CopyFrom(const ProgressEventTypeData_t& source);
...
private:
	...
	IntrusivePtr_t<UInt16_t> progress;
	
	

public:
	//Accessors
	IntrusivePtr_t<const BaseDataType_t> Context(void) const {return context;}
	
	IntrusivePtr_t<BaseDataType_t> &Context(void) {return context;}
	
	
	
	IntrusivePtr_t<const UInt16_t> Progress(void) const {return progress;}
	
	IntrusivePtr_t<UInt16_t> &Progress(void) {return progress;}
...
#endif /* _PROGRESSEVENTTYPEDATA_T_H_ */
