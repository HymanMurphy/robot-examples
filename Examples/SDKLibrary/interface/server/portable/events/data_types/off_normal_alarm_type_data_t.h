...
#ifndef _OFFNORMALALARMTYPEDATA_T_H_
#define _OFFNORMALALARMTYPEDATA_T_H_
...
class OffNormalAlarmTypeData_t : public DiscreteAlarmTypeData_t
{
public:
	UA_DECLARE_RUNTIME_TYPE(OffNormalAlarmTypeData_t);
	
	static const uint32_t TYPE_ID = OpcUaId_OffNormalAlarmType;
	
	OffNormalAlarmTypeData_t();
	virtual ~OffNormalAlarmTypeData_t();
	
	virtual Status_t CopyTo(IntrusivePtr_t<BaseDataType_t>& destination) const;
	virtual Status_t CopyFrom(const BaseDataType_t& source);
	Status_t CopyFrom(const OffNormalAlarmTypeData_t& source);
...
private:
	...
	IntrusivePtr_t<NodeId_t> normalState;
	
	

public:
	//Accessors
	IntrusivePtr_t<const NodeId_t> NormalState(void) const {return normalState;}
	
	IntrusivePtr_t<NodeId_t> &NormalState(void) {return normalState;}
...
#endif /* _OFFNORMALALARMTYPEDATA_T_H_ */
