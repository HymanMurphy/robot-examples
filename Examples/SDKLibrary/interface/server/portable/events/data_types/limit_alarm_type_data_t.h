...
#ifndef _LIMITALARMTYPEDATA_T_H_
#define _LIMITALARMTYPEDATA_T_H_
...
class LimitAlarmTypeData_t : public AlarmConditionTypeData_t
{
public:
	UA_DECLARE_RUNTIME_TYPE(LimitAlarmTypeData_t);
	
	static const uint32_t TYPE_ID = OpcUaId_LimitAlarmType;
	
	LimitAlarmTypeData_t();
	virtual ~LimitAlarmTypeData_t();
	
	virtual Status_t CopyTo(IntrusivePtr_t<BaseDataType_t>& destination) const;
	virtual Status_t CopyFrom(const BaseDataType_t& source);
	Status_t CopyFrom(const LimitAlarmTypeData_t& source);
...
private:
	...
	IntrusivePtr_t<Double_t> highHighLimit;
	
	IntrusivePtr_t<Double_t> highLimit;
	
	IntrusivePtr_t<Double_t> lowLimit;
	
	IntrusivePtr_t<Double_t> lowLowLimit;
	
	

public:
	//Accessors
	IntrusivePtr_t<const Double_t> HighHighLimit(void) const {return highHighLimit;}
	
	IntrusivePtr_t<Double_t> &HighHighLimit(void) {return highHighLimit;}
	
	
	
	IntrusivePtr_t<const Double_t> HighLimit(void) const {return highLimit;}
	
	IntrusivePtr_t<Double_t> &HighLimit(void) {return highLimit;}
	
	
	
	IntrusivePtr_t<const Double_t> LowLimit(void) const {return lowLimit;}
	
	IntrusivePtr_t<Double_t> &LowLimit(void) {return lowLimit;}
	
	
	
	IntrusivePtr_t<const Double_t> LowLowLimit(void) const {return lowLowLimit;}
	
	IntrusivePtr_t<Double_t> &LowLowLimit(void) {return lowLowLimit;}
...
#endif /* _LIMITALARMTYPEDATA_T_H_ */
