...
#ifndef _TRIPALARMTYPEDATA_T_H_
#define _TRIPALARMTYPEDATA_T_H_
...
class TripAlarmTypeData_t : public OffNormalAlarmTypeData_t
{
public:
	UA_DECLARE_RUNTIME_TYPE(TripAlarmTypeData_t);
	
	static const uint32_t TYPE_ID = OpcUaId_TripAlarmType;
	
	TripAlarmTypeData_t();
	virtual ~TripAlarmTypeData_t();
	
	virtual Status_t CopyTo(IntrusivePtr_t<BaseDataType_t>& destination) const;
	virtual Status_t CopyFrom(const BaseDataType_t& source);
	Status_t CopyFrom(const TripAlarmTypeData_t& source);
