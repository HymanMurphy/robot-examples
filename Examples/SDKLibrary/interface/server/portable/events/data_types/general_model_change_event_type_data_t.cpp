...
UA_DEFINE_RUNTIME_TYPE(GeneralModelChangeEventTypeData_t, BaseModelChangeEventTypeData_t);

GeneralModelChangeEventTypeData_t::GeneralModelChangeEventTypeData_t()
{
}
...
bool GeneralModelChangeEventTypeData_t::IsOfType(const uint32_t typeId, uint16_t namespaceindex) const
{
	if(namespaceindex != 0) return false;
	return typeId == 2133 || BaseModelChangeEventTypeData_t::IsOfType(typeId, namespaceindex);
}

Status_t GeneralModelChangeEventTypeData_t::Initialise()
{
	Status_t rtnStatus = OpcUa_Good;
	browseName = new SafeRefCount_t<String_t>();
	UASDK_RETURN_OUT_OF_MEMORY_IF_NULL(browseName);
	rtnStatus = browseName->CopyFrom("GeneralModelChangeEventType");
...
uint32_t GeneralModelChangeEventTypeData_t::TypeId(uint16_t& namespaceIndex) const
{
	namespaceIndex = 0;
	return GeneralModelChangeEventTypeData_t::TYPE_ID;
}

IntrusivePtr_t<BaseDataType_t> GeneralModelChangeEventTypeData_t::AttributeValue(const uint32_t& typeId, const ArrayUA_t<QualifiedName_t>&browsePath, const uint32_t &_attributeId, const String_t& _indexRange, const ArrayUA_t<String_t> & locales,  Status_t &result)
{
	...
	if(UASDK_strcmp(nameStr, "Changes.Verb") == 0 && _attributeId == AttributeId_t::ATTRIBUTE_ID_VALUE) //From 1
	...
	IntrusivePtr_t<const BaseDataType_t> GeneralModelChangeEventTypeData_t::AttributeValue(const uint32_t& typeId, const ArrayUA_t<QualifiedName_t>&browsePath, const uint32_t &_attributeId, const String_t& _indexRange, const ArrayUA_t<String_t> & locales, Status_t &result) const
	{
	...
	Status_t GeneralModelChangeEventTypeData_t::CopyTo(IntrusivePtr_t<BaseDataType_t>& destination) const
	{
		return CopyToDestination(*this, destination);
	}
	
	Status_t GeneralModelChangeEventTypeData_t::CopyFrom(const BaseDataType_t& source)
	{
		IntrusivePtr_t<const GeneralModelChangeEventTypeData_t> ptr = RuntimeCast<const GeneralModelChangeEventTypeData_t*>(source);
		if (!ptr.is_set())
		{
	...
	Status_t GeneralModelChangeEventTypeData_t::CopyFrom(const GeneralModelChangeEventTypeData_t& source)
	{
		...
		if(source.changes_Verb.is_set())
		
		{
		
		changes_Verb.reset();
		
		  changes_Verb = new SafeRefCount_t<Byte_t>();
		
		  if(!changes_Verb.is_set())
		  ...
		  result = changes_Verb->CopyFrom(*source.Changes_Verb());
